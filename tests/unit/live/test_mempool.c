...
TEST_TEAR_DOWN(lwpa_mempool)
{
}

TEST(lwpa_mempool, alloc_and_free_works)
{
  // Initialize the pool.
  TEST_ASSERT_EQUAL(kLwpaErrOk, lwpa_mempool_init(alloc_test));
  TEST_ASSERT_EQUAL_UINT(ALLOC_TEST_MEMP_SIZE, lwpa_mempool_size(alloc_test));

  // Allocate the entire pool.
  for (size_t i = 0; i < ALLOC_TEST_MEMP_SIZE; ++i)
  {
    TestElem* elem = (TestElem*)lwpa_mempool_alloc(alloc_test);
    TEST_ASSERT(elem);
    test_arr[i] = elem;
  }
  TEST_ASSERT_EQUAL_UINT(ALLOC_TEST_MEMP_SIZE, lwpa_mempool_used(alloc_test));
  ...
  for (size_t i = 0; i < ALLOC_TEST_MEMP_SIZE; ++i)
  {
    lwpa_mempool_free(alloc_test, test_arr[index_arr[i]]);
  }
  TEST_ASSERT_EQUAL_UINT(0u, lwpa_mempool_used(alloc_test));

  // Make sure we can allocate the entire pool again.
  for (size_t i = 0; i < ALLOC_TEST_MEMP_SIZE; ++i)
  {
    test_arr[i] = (TestElem*)lwpa_mempool_alloc(alloc_test);
    TEST_ASSERT(test_arr[i]);
  }
}

TEST(lwpa_mempool, alloc_and_free_array_works)
{
  // Initialize the pool.
  TEST_ASSERT_EQUAL(kLwpaErrOk, lwpa_mempool_init(alloc_array_test));
  TEST_ASSERT_EQUAL_UINT(ALLOC_TEST_MEMP_SIZE, lwpa_mempool_size(alloc_array_test));

  // Allocate the entire pool.
  for (size_t i = 0; i < ALLOC_TEST_MEMP_SIZE; ++i)
  {
    TestElem* elem_arr = (TestElem*)lwpa_mempool_alloc(alloc_array_test);
  ...
  TEST_ASSERT_EQUAL_UINT(ALLOC_TEST_MEMP_SIZE, lwpa_mempool_used(alloc_array_test));
  ...
  for (size_t i = 0; i < ALLOC_TEST_MEMP_SIZE; ++i)
  {
    lwpa_mempool_free(alloc_array_test, test_arr[index_arr[i]]);
  }
  TEST_ASSERT_EQUAL_UINT(0u, lwpa_mempool_used(alloc_array_test));

  // Make sure we can allocate the entire pool again.
  for (size_t i = 0; i < ALLOC_TEST_MEMP_SIZE; ++i)
  {
    test_arr[i] = (TestElem*)lwpa_mempool_alloc(alloc_array_test);
...
TEST_GROUP_RUNNER(lwpa_mempool)
{
