...
{
public:
  explicit SynchronizedRef(const Synchronized<T, Lock>& value) noexcept : value_{std::addressof(value)}
  {
    value_->mutex_.lock();
  }
...
{
public:
  explicit SynchronizedRef(Synchronized<T, Lock>& value) noexcept : value_{std::addressof(value)}
  {
