...
///
/// Provides a thread class Thread which is similar to std::thread, with the key
/// advantage that it will work on any threaded platform that EtcPal is ported
/// for, including the embedded RTOS platforms. If your application or library
/// does not need to run on these platforms, consider using std::thread instead
/// (or in >= C++20, std::jthread, which more closely matches the behavior of
/// this class).
///
...
/// // Thread will be automatically joined when thread goes out of scope, or
/// join it explicitly: thread.Join();
/// @endcode
///
/// The value constructor will throw an etcpal::Error object if the thread fails
/// to start for any reason. To avoid an exception-handling approach, you can
/// default-construct and use the Start() function:
/// @code
...
/// @endcode
///
/// This approach also lets you set non-default thread parameters before
/// starting; method-chaining syntax is available on these setters:
/// @code
/// etcpal::Thread thread;
...
/// }
///
/// etcpal::Thread thread(MyThreadFunction, 5); // Thread will print "The value
/// is 5."
/// @endcode
///
...
/// @endcode
///
/// The Thread class also provides static Sleep() functions, which can be used
/// as a platform-neutral sleep.
/// @code
...
/// @endcode
///
/// **IMPORTANT NOTE FOR RTOS USERS:** The EtcPal threading API does not
/// initialize the scheduler on real-time systems (e.g. it does not call
/// vTaskStartScheduler() on FreeRTOS or _mqx() on MQX). The application writer
/// is responsible for managing the interaction between starting the scheduler
/// and starting EtcPal threads, just like when starting native RTOS tasks.

/// @ingroup etcpal_cpp_thread
/// @brief A thread class, modeled after std::thread.
///
/// This class is similar to std::thread, with the key advantage that it will
/// work on any threaded platform that EtcPal is ported for, including the
/// embedded RTOS platforms. If your application or library does not need to run
/// on these platforms, consider using std::thread instead.
///
/// There are a few differences from std::thread:
/// * The thread is automatically joined on destruction (this matches the
/// behavior of C++20's
///   std::jthread)
/// * The thread is not _detachable_; that is, once created, it _must_ be joined
/// before it is
///   destroyed in all cases.
/// * The thread has a Start() function - a previously default-constructed
/// thread can be assigned
///   some operation parameters, then started, without being reassigned to
///   another instance.
///
/// This is one of the few EtcPal wrappers that does heap allocation, as the
/// thread function and arguments need to be stored.
class Thread {
public:
  ...
  Thread(unsigned int priority, unsigned int stack_size, const char *name,
  ...
  static void SleepUs(unsigned int us) noexcept;
  template <typename Rep, typename Period>
  static void
  Sleep(const std::chrono::duration<Rep, Period> &sleep_duration) noexcept;
...
extern "C" inline void CppThreadFn(void *arg) {
  std::unique_ptr<Thread::FunctionType> p_func(
      static_cast<Thread::FunctionType *>(arg));
...
/// @brief Create a new thread object and associate it with a new thread of
/// execution.
///
/// Default thread parameters will be used. See the Start() function for more
/// information.
///
...
template <
    ...
    typename std::enable_if<!std::is_arithmetic<Function>::value, bool>::type>
Thread::Thread(Function &&func, Args &&... args) {
  ...
  if (!result)
    ETCPAL_THROW(std::runtime_error("Error while starting EtcPal thread: " +
                                    result.ToString()));
}
...
/// @param name A name for the thread, maximum length
/// #ETCPAL_THREAD_NAME_MAX_LENGTH.
/// @param platform_data Pointer to a platform-specific parameter structure. See
///                      #EtcPalThreadParams for more information.
inline Thread::Thread(unsigned int priority, unsigned int stack_size,
                      const char *name, void *platform_data)
    : params_{priority, stack_size, name, platform_data} {}

/// @brief Destroy the thread object.
///
/// If *this has a valid associated thread (`joinable() == true`), the thread is
/// joined before the destructor finishes.
...
///
/// If *this has a valid associated thread (`joinable() == true`), the behavior
/// is undefined. After this call, *this has the parameters of other, and other
/// is set to a default-constructed state.
inline Thread::Thread(Thread &&other) noexcept { *this = std::move(other); }
...
inline bool Thread::joinable() const noexcept { return (bool)thread_; }
...
/// @brief Get the stack size of this thread in bytes (not valid on all
/// platforms).
...
inline const char *Thread::name() const noexcept { return params_.thread_name; }
...
/// @return The thread's OS handle, or ETCPAL_THREAD_OS_HANDLE_INVALID if the
/// thread is not running
///         (`joinable() == false`).
inline etcpal_thread_os_handle_t Thread::os_handle() const noexcept {
  return thread_ ? etcpal_thread_get_os_handle(thread_.get())
                 : ETCPAL_THREAD_OS_HANDLE_INVALID;
}

/// @brief Set the priority of this thread.
///
/// Priority is not valid on all platforms. This function does not have any
/// effect on the associated thread unless it is called on a default-constructed
/// thread before Start() is called.
///
...
///
/// Stack size is not valid on all platforms. This function does not have any
/// effect on the associated thread unless it is called on a default-constructed
/// thread before Start() is called.
///
...
///
/// This function does not have any effect on the associated thread unless it is
/// called on a default-constructed thread before Start() is called. The pointer
/// passed to this function must remain valid until after Start() is called.
///
...
///
/// This function does not have any effect on the associated thread unless it is
/// called on a default-constructed thread before Start() is called. The string
/// reference passed to this function must remain valid until after Start() is
/// called.
///
/// @param name Name to set.
/// @return A reference to this thread, for method chaining.
...
/// @brief Set this thread's parameters from an existing EtcPalThreadParams
/// struct.
///
/// This function does not have any effect on the associated thread unless it is
/// called on a default-constructed thread before Start() is called.
///
...
/// @endcode
/// Except that the calls to decay_copy are evaluated in the context of the
/// caller, so that any exceptions thrown during evaluation and copying/moving
/// of the arguments are thrown in the current thread, without starting the new
/// thread.
///
/// @param func Callable object to execute in the new thread.
/// @param args Arguments to pass to func.
/// @return #kEtcPalErrOk: The thread started successfully, `joinable()` is now
/// true.
/// @return #kEtcPalErrNoMem: Allocation failed while starting thread.
/// @return #kEtcPalErrInvalid: The thread was already running (`joinable() ==
/// true`).
...
Error Thread::Start(Function &&func, Args &&... args) {
  ...
  Error create_res =
      etcpal_thread_create(thread_.get(), &params_, CppThreadFn, new_f.get());
  if (create_res)
    new_f.release();
  else
    thread_.reset();
...
///
/// If timeout_ms is not given, or if #ETCPAL_THREAD_HAS_TIMED_JOIN is false on
/// this platform, blocks until the thread has exited. Otherwise, blocks up to
/// timeout_ms waiting for the thread to exit.
///
...
/// @return #kEtcPalErrTimedOut: The thread did not join within the specified
/// timeout.
/// @return #kEtcPalErrInvalid: The thread was not running (`joinable() ==
/// false`).
/// @return Other codes translated from system error codes are possible.
...
///
/// **Be careful when using this function.** Depending on the state of a thread
/// when it is terminated, shared resources or memory could not be freed,
/// resulting in memory leaks or deadlocks.
///
...
inline void Thread::Sleep(unsigned int ms) noexcept { etcpal_thread_sleep(ms); }


/// @brief Blocks the current thread for the specified number of microseconds.
///
/// This function is only defined when the macro etcpal_thread_sleep_us is defined
/// (usually in the os_thread.h file).
#if defined(etcpal_thread_sleep_us)
inline void Thread::SleepUs(unsigned int us) noexcept {
  etcpal_thread_sleep_us(us);
}
#endif // defined(etcpal_thread_sleep_us)
