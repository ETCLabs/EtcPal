...
// These are defined before the includes to enable IPV6_PKTINFO support.
#ifndef _GNU_SOURCE
#define _GNU_SOURCE
#endif
...
{
  if (!ETCPAL_ASSERT_VERIFY(kevents))
    return 0;
...
{
  if (!ETCPAL_ASSERT_VERIFY(kevent) || !ETCPAL_ASSERT_VERIFY(sock_desc))
    return (etcpal_poll_events_t)0;
