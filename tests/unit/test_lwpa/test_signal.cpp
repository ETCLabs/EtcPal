...
TEST_F(SignalTest, create_destroy)
{
  ...
  ASSERT_FALSE(lwpa_signal_poll(&signal));

  lwpa_signal_post(&signal);
  ASSERT_TRUE(lwpa_signal_wait(&signal));

  //  auto start_time = std::chrono::high_resolution_clock::now();
  //  // Signal wait should fail if not signaled
  //  ASSERT_FALSE(lwpa_signal_timed_wait(&signal, 100));
  ...
  ASSERT_FALSE(lwpa_signal_wait(&signal));
}
