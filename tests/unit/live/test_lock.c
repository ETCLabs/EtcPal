...
TEST(etcpal_lock, sem_try_wait_works)
{
  etcpal_sem_t sem;
  TEST_ASSERT_TRUE(etcpal_sem_create(&sem, 1, 10));
...
TEST(etcpal_lock, sem_timed_wait_works)
{
  etcpal_sem_t sem;
...
TEST(etcpal_lock, sem_max_count)
{
  etcpal_sem_t sem;
...
TEST_GROUP_RUNNER(etcpal_lock)
{
  ...
  RUN_TEST_CASE(etcpal_lock, sem_counting_works);
  RUN_TEST_CASE(etcpal_lock, sem_try_wait_works);
  RUN_TEST_CASE(etcpal_lock, sem_timed_wait_works);
#if ETCPAL_SEM_HAS_MAX_COUNT
